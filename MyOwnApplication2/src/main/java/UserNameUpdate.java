	...
	private static final long serialVersionUID = 1L;
    	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			try
			{
				...
				String query1="update users set user_name=? where user_name=?";
				PreparedStatement ps=con.prepareStatement(query1);
				ps.setString(1, user_name2);
				ps.setString(2, user_name);
				ResultSet rs1=ps.executeQuery();
					if(rs1.next())
					{
						out.println("Your User_Name Updated SuccessFully");
					}
					else
					{
						out.println("Your User_Name not Updated");
					}
				con.close();
			}
			catch(Exception e)
			{
				out.println("Your User_Name not Updated/Record Not Found");
